...
export default function MyStars({ id, rate, from, change }) {
  ...
  return (
    <div className='rating-area'>
      ...
      {[5, 4, 3, 2, 1].map((ind) => 
        <>
          <input key={`${id}${ind}`} type="radio" id={`star-${ind}${id}`} name={'rating' + id} value={`${ind}`} disabled={dis} checked={checkedInput[Number(ind)]} onClick={(e) => addRate(e.target.value)} onChange={() => { return false; }}></input>
